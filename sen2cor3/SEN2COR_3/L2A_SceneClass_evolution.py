...
class L2A_SceneClass(object):
    def __init__(self, config, tables):
        ...
        x = self.config.nrows
        y = self.config.ncols
        self.classificationMask = ones_like(tables.getBand(Band.BLUE), dtype=ubyte) * self._notClassified #self.tables.B02
    ...
    def preprocess(self):
        ...
        if 'LANDSAT' not in self.config.spacecraftName:
            if self.config.qltmask is True and self.tables.hasBand(Band.QUALITY_MASK) is True:
    ...
    def postprocess(self):
        ...
        self.tables.setBand(Band.SCENE_CLASSIFICATION, (CM).astype(uint8)) #self.tables.SCL
        self.logger.info('Storing final Snow Confidence Mask')
        self.tables.setBand(Band.SNOW_MAP, (self.confidenceMaskSnow*100+0.5).astype(uint8)) #elf.tables.SNW
        self.logger.info('Storing final Cloud Confidence Mask')
        self.tables.setBand(Band.CLOUD_MAP, (self.confidenceMaskCloud*100+0.5).astype(uint8)) #self.tables.CLD
    ...
    def postprocessnew(self):
        ...
        def filter_isolated_cells(image, npix, struct):
            ...
            id_sizes = array(ndimage.sum(image, id_regions, list(range(num_ids + 1))))
        ...
        WBI = self.tables.getBand(Band.WATER_BODY_INDEX) #self.tables.WBI
        ...
        LCM = self.tables.getBand(Band.LAND_COVERAGE_MAP) #self.tables.LCM
        ...
        self.tables.setBand(Band.SCENE_CLASSIFICATION, (CM).astype(uint8)) #self.tables.SCL
        self.logger.info('Storing final Snow Confidence Mask')
        self.tables.setBand(Band.SNOW_MAP, (self.confidenceMaskSnow*100+0.5).astype(uint8)) #self.tables.SNW
        self.logger.info('Storing final Cloud Confidence Mask')
        self.tables.setBand(Band.CLOUD_MAP, (self.confidenceMaskCloud*100+0.5).astype(uint8)) #self.tables.CLD
    ...
    def L2A_CSND_1_1(self):
        ...
        B04 = self.tables.getBand(Band.RED)  #self.tables.B04
        if 'LANDSAT' in self.config.spacecraftName:
    ...
    def L2A_CSND_1_2(self):
        ...
        B03 = self.tables.getBand(Band.GREEN) # self.tables.B03
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1) # self.tables.B11
    ...
    def L2A_CSND_2_1(self):
        ...
        if self.tables.hasBand(Band.SNOW_CONDITION_MAP) is True: # self.tables.SNC
            SNC = self.tables.getBand(Band.SNOW_CONDITION_MAP) # self.tables.SNC
    ...
    def L2A_CSND_2_1bis(self):
        ...
        if 'LANDSAT' in self.config.spacecraftName:
            B05 = self.tables.getBand(Band.RED)
    ...
    def L2A_CSND_2_3(self):
        ...
        B02 = self.tables.getBand(Band.BLUE) #self.tables.B02
    ...
    def L2A_CSND_2_4(self):
        ...
        B02 = self.tables.getBand(Band.BLUE) #s elf.tables.B02
        B04 = self.tables.getBand(Band.RED) # self.tables.B04
    ...
    def L2A_CSND_2_5(self):
        ...
        B12 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_2) # self.tables.B12
        CM = self.classificationMask
    ...
    def L2A_CSND_5_2(self):
        ...
        B04 = self.tables.getBand(Band.RED) # B04 used for additional condition #self.tables.B04
    ...
    def L2A_CSND_7(self):
        ...
        B02 = self.tables.getBand(Band.BLUE) #self.tables.B02
        B10 = self.tables.getBand(Band.CIRRUS) #self.tables.B10
        ...
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) is True): #self.tables.DEM
            dem = self.tables.getBand(Band.DIGITAL_ELEVATION_MAP) #self.tables.DEM
        else:
    ...
    def L2A_WaterPixelCleaningwithDEM(self):
        ...
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) is True): #self.tables.DEM
            slope = self.tables.getBand(Band.SLOPE) #self.tables.SLP
            shadow = self.tables.getBand(Band.SHADOW_MAP) #self.tables.SDW
            T_Shadow = 1
            T_Slope = 15
    ...
    def L2A_SnowRecovery(self):
        B03 = self.tables.getBand(Band.RED) #self.tables.B03
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1) #self.tables.B11
        CM = self.classificationMask
    ...
    def L2A_CirrusPixelRecoveryB10(self):
        if (self.tables.hasBand(Band.LAND_COVERAGE_MAP) is True): #self.tables.LCM
            #LCM = self.tables.getBand(self.tables.LCM)
            CM = self.classificationMask
            B10 = self.tables.getBand(Band.CIRRUS) #self.tables.B10
            ...
            if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) is True): #self.tables.DEM
                dem = self.tables.getBand(Band.DIGITAL_ELEVATION_MAP) #self.tables.DEM
                cbeta = self.tables.getBand(Band.SHADOW_MAP).astype(float32) / 255.0 #self.tables.SDW
    ...
    def L2A_Bright_pixels_Hollstein_Fmask(self):
        ...
        B01 = self.tables.getBand(Band.COASTAL_AEROSOL) #self.tables.B01
        B03 = self.tables.getBand(Band.GREEN) #self.tables.B03
        if 'LANDSAT' in self.config.spacecraftName:
            B05 = self.tables.getBand(Band.RED) #self.tables.B05 ?? #is this correct? #francesco
        else:
            B05 = self.tables.getBand(Band.VEGETATION_1) #self.tables.B05
        B10 = self.tables.getBand(Band.CIRRUS) #self.tables.B10
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1) #self.tables.B11
        CM = self.classificationMask
        ...
        if self.tables.hasBand(Band.WATER_BODY_INDEX) is True: #self.tables.WBI
            WBI = self.tables.getBand(Band.WATER_BODY_INDEX) #self.tables.WBI
    ...
    def L2A_CSHD_1(self, csd2):
        ...
        BX[0,:,:] = self.tables.getBand(Band.BLUE) # self.tables.B02
        BX[1,:,:] = self.tables.getBand(Band.GREEN) # self.tables.B03
        BX[2,:,:] = self.tables.getBand(Band.RED) # self.tables.B04
        if 'LANDSAT' in self.config.spacecraftName:
            BX[3, :, :] = self.tables.getBand(Band.NEAR_INFRARED)
        else:
            BX[3, :, :] = self.tables.getBand(Band.VEGETATION_4)
        BX[4,:,:] = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1) # self.tables.B11
        BX[5,:,:] = self.tables.getBand(Band.SHORT_WAVE_INFRARED_2) # self.tables.B12
        ...
        # or if now pixels represents more than 5% of data Version 2.10.2
        ...
        # and sza < 45
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) is True) & (self.config._solze_noclip < 45): #self.tables.DEM
            cbeta = self.tables.getBand(Band.SHADOW_MAP)/255. #self.tables.SDW
            T_lsza = 5.0 # corresponds to a variation of T_lsza deg of local solar zenith angle
            sza = self.config._solze_noclip
            sza_T_lsza = clip(sza + T_lsza, 0, 90)
            lsza_plus_5deg = cbeta > cos(sza_T_lsza * pi / 180.)
    ...
    def L2A_CSHD_2_KLT_GAUSSIAN_BIS(self):
        ...
        if not self.config.Hyper:
            ...
            filename = [tiffile for tiffile in dirs if 'B08.TIF' in tiffile][0]
            filename = os.path.join(sourceDir, filename)
        ...
        if not self.config.Hyper:
            if self.config.resolution == 20:
                ...
                img_box = uint16(block_reduce(indataArr, block_size=(3, 3), func=mean) + 0.5)

        if 'LANDSAT' in self.config.spacecraftName:
                ref_box = (10000.0 * self.tables.getBand(Band.NEAR_INFRARED)).astype(uint16)
        else:
                ref_box = (10000.0 * self.tables.getBand(Band.VEGETATION_4)).astype(uint16)
        # ref_box = (10000.0 * self.tables.getBand(self.tables.B8A)).astype(uint16) #above it was like this
        ...
        if (KLTMask.sum() / float(CM.size)) > 0.02: # Version 2.10.2
            ...
            if cloud_height.size > 0:
                ...
                if (cloud_mask.sum() / float(KLTMask.sum())) < 0.95: # Version 2.10.2
        ...
        #     # filt_b = zeros([distr_shad.size, x_aa], float32) #old
        #     # filt_b[0:distr_shad.size, 0] = distr_shad #old
    ...
    def process(self):
        ...
        self.config.timestamp('Scene Classification Evolution   ')
        ...
        dilate = True
        if dilate & (self.tables.hasBand(Band.LAND_COVERAGE_MAP) is True) & (self.tables.hasBand(Band.WATER_BODY_INDEX) is True): #self.tables.LCM, self.tables.WBI
