...
# This file includes software developed at Telespazio France.
...
class L2A_SceneClass(object):
    def __init__(self, config, tables):
        ...
        x = self.config.nrows
        y = self.config.ncols
        self.classificationMask = ones_like(tables.getBand(Band.BLUE), dtype=ubyte) * self._notClassified
        ...
        self._noData = int32(par.pyval)

        par = node.SATURATED_DEFECTIVE
        if par is None: self.parNotFound(node)
        self._saturatedDefective = int32(par.pyval)

        par = node.DARK_FEATURES
        if par is None: self.parNotFound(node)
        self._darkFeatures = int32(par.pyval)

        par = node.CLOUD_SHADOWS
        if par is None: self.parNotFound(node)
        self._cloudShadows = int32(par.pyval)

        par = node.VEGETATION
        if par is None: self.parNotFound(node)
        self._vegetation = int32(par.pyval)

        par = node.NOT_VEGETATED
        if par is None: self.parNotFound(node)
        self._bareSoils = int32(par.pyval)

        par = node.WATER
        if par is None: self.parNotFound(node)
        self._water = int32(par.pyval)

        par = node.UNCLASSIFIED
        if par is None: self.parNotFound(node)
        self._lowProbaClouds = int32(par.pyval)

        par = node.MEDIUM_PROBA_CLOUDS
        if par is None: self.parNotFound(node)
        self._medProbaClouds = int32(par.pyval)

        par = node.HIGH_PROBA_CLOUDS
        if par is None: self.parNotFound(node)
        self._highProbaClouds = int32(par.pyval)

        par = node.THIN_CIRRUS
        if par is None: self.parNotFound(node)
        self._thinCirrus = int32(par.pyval)

        par = node.SNOW_ICE
        if par is None: self.parNotFound(node)
        self._snowIce = int32(par.pyval)
        ...
        par = node.T1_B02
        if par is None: self.parNotFound(node)
        self._T1_B02 = float32(par.pyval)

        par = node.T2_B02
        if par is None: self.parNotFound(node)
        self._T2_B02 = float32(par.pyval)

        par = node.T1_B04
        if par is None: self.parNotFound(node)
        self._T1_B04 = float32(par.pyval)

        par = node.T2_B04
        if par is None: self.parNotFound(node)
        self._T2_B04 = float32(par.pyval)

        par = node.T1_B8A
        if par is None: self.parNotFound(node)
        self._T1_B8A = float32(par.pyval)

        par = node.T2_B8A
        if par is None: self.parNotFound(node)
        self._T2_B8A = float32(par.pyval)

        par = node.T1_B10
        if par is None: self.parNotFound(node)
        self._T1_B10 = float32(par.pyval)

        par = node.T2_B10
        if par is None: self.parNotFound(node)
        self._T2_B10 = float32(par.pyval)

        par = node.T1_B12
        if par is None: self.parNotFound(node)
        self._T1_B12 = float32(par.pyval)

        par = node.T2_B12
        if par is None: self.parNotFound(node)
        self._T2_B12 = float32(par.pyval)

        par = node.T1_NDSI_CLD
        if par is None: self.parNotFound(node)
        self._T1_NDSI_CLD = float32(par.pyval)

        par = node.T2_NDSI_CLD
        if par is None: self.parNotFound(node)
        self._T2_NDSI_CLD = float32(par.pyval)

        par = node.T1_NDSI_SNW
        if par is None: self.parNotFound(node)
        self._T1_NDSI_SNW = float32(par.pyval)

        par = node.T2_NDSI_SNW
        if par is None: self.parNotFound(node)
        self._T2_NDSI_SNW = float32(par.pyval)

        par = node.T1_R_B02_B04
        if par is None: self.parNotFound(node)
        self._T1_R_B02_B04 = float32(par.pyval)

        par = node.T2_R_B02_B04
        if par is None: self.parNotFound(node)
        self._T2_R_B02_B04 = float32(par.pyval)

        par = node.T1_R_B8A_B03
        if par is None: self.parNotFound(node)
        self._T1_R_B8A_B03 = float32(par.pyval)

        par = node.T2_R_B8A_B03
        if par is None: self.parNotFound(node)
        self._T2_R_B8A_B03 = float32(par.pyval)

        par = node.T1_R_B8A_B11
        if par is None: self.parNotFound(node)
        self._T1_R_B8A_B11 = float32(par.pyval)

        par = node.T2_R_B8A_B11
        if par is None: self.parNotFound(node)
        self._T2_R_B8A_B11 = float32(par.pyval)

        par = node.T1_SNOW
        if par is None: self.parNotFound(node)
        self._T1_SNOW = float32(par.pyval)

        par = node.T2_SNOW
        if par is None: self.parNotFound(node)
        self._T2_SNOW = float32(par.pyval)

        par = node.T1_NDVI
        if par is None: self.parNotFound(node)
        self._T1_NDVI = float32(par.pyval)

        par = node.T2_NDVI
        if par is None: self.parNotFound(node)
        self._T2_NDVI = float32(par.pyval)
        ...
        par = node.T11_B02
        if par is None: self.parNotFound(node)
        self._T11_B02 = float32(par.pyval)

        par = node.T12_B02
        if par is None: self.parNotFound(node)
        self._T12_B02 = float32(par.pyval)

        par = node.T11_R_B02_B11
        if par is None: self.parNotFound(node)
        self._T11_R_B02_B11 = float32(par.pyval)

        par = node.T12_R_B02_B11
        if par is None: self.parNotFound(node)
        self._T12_R_B02_B11 = float32(par.pyval)

        par = node.T21_B12
        if par is None: self.parNotFound(node)
        self._T21_B12 = float32(par.pyval)

        par = node.T22_B12
        if par is None: self.parNotFound(node)
        self._T22_B12 = float32(par.pyval)

        par = node.T21_R_B02_B11
        if par is None: self.parNotFound(node)
        self._T21_R_B02_B11 = float32(par.pyval)

        par = node.T22_R_B02_B11
        if par is None: self.parNotFound(node)
        self._T22_R_B02_B11 = float32(par.pyval)

        par = node.T_CLOUD_LP
        if par is None: self.parNotFound(node)
        self._T_CLOUD_LP = float32(par.pyval)

        par = node.T_CLOUD_MP
        if par is None: self.parNotFound(node)
        self._T_CLOUD_MP = float32(par.pyval)

        par = node.T_CLOUD_HP
        if par is None: self.parNotFound(node)
        self._T_CLOUD_HP = float32(par.pyval)
        ...
        par = node.T_SDW
        if par is None: self.parNotFound(node)
        self._T_SDW = float32(par.pyval)

        par = node.T_B02_B12
        if par is None: self.parNotFound(node)
        self._T_B02_B12 = float32(par.pyval)
    ...
    def preprocess(self):
        ...
        if 'LANDSAT' not in self.config.spacecraftName:
            if self.config.qltmask == True and self.tables.hasBand(Band.QUALITY_MASK) == True :
                quality_mask=self.tables.getBand(Band.QUALITY_MASK) #self.tables.QMS
        ...
        bands = self.tables.bands
        for identifier in bands:
            band = self.tables.getBand(identifier)
            if 'LANDSAT' in self.config.spacecraftName:
               # CM[isnan(band)] = self._noData
    ...
    def postprocess(self):
        ...
        self.tables.setBand(Band.SCENE_CLASSIFICATION, (CM).astype(uint8))
        self.logger.info('Storing final Snow Confidence Mask')
        self.tables.setBand(Band.SNOW_MAP, (self.confidenceMaskSnow*100+0.5).astype(uint8))
        self.logger.info('Storing final Cloud Confidence Mask')
        self.tables.setBand(Band.CLOUD_MAP, (self.confidenceMaskCloud*100+0.5).astype(uint8))
        if 'LANDSAT' not in self.config.operationMode:
    ...
    def L2A_CSND_1_1(self):
        ...
        B04 = self.tables.getBand(Band.RED)
        if 'LANDSAT' in self.config.spacecraftName:
            B08 = self.tables.getBand(Band.NEAR_INFRARED)
        else:
            B08 = self.tables.getBand(Band.VEGETATION_4)
    ...
    def L2A_CSND_1_2(self):
        ...
        T1_NDSI_CLD = self._T1_NDSI_CLD
        T2_NDSI_CLD = self._T2_NDSI_CLD
        #JL20151217 f1 = self.confidenceMaskCloud > 0
        B03 = self.tables.getBand(Band.GREEN)
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
    ...
    def L2A_CSND_2_1(self):
        ...
        T1_NDSI_SNW = self._T1_NDSI_SNW
        T2_NDSI_SNW = self._T2_NDSI_SNW
        B03 = self.tables.getBand(Band.GREEN)
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
        NDSI = (B03 - B11) / maximum((B03 + B11), self.LOWEST)
        ...
        if self.tables.hasBand(Band.SNOW_CONDITION_MAP) == True:
            SNC = self.tables.getBand(Band.SNOW_CONDITION_MAP)
    ...
    def L2A_CSND_2_1bis(self):
        ...
        if 'LANDSAT' in self.config.spacecraftName:
            B05 = self.tables.getBand(Band.RED) #is this correct? #francesco
            B8A = self.tables.getBand(Band.NEAR_INFRARED)
        else:
            B05 = self.tables.getBand(Band.VEGETATION_1) #
            B8A = self.tables.getBand(Band.VEGETATION_4)
    ...
    def L2A_CSND_2_3(self):
        ...
        T1_B02 = self._T1_B02
        T2_B02 = self._T2_B02
        B02 = self.tables.getBand(Band.BLUE)
    ...
    def L2A_CSND_2_4(self):
        ...
        T1_R_B02_B04 = self._T1_R_B02_B04
        T2_R_B02_B04 = self._T2_R_B02_B04
        B02 = self.tables.getBand(Band.BLUE)
        B04 = self.tables.getBand(Band.RED)
    ...
    def L2A_CSND_2_5(self):
        ...
        T1_SNOW = self._T1_SNOW
        T2_SNOW = self._T2_SNOW
        B12 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_2)
        CM = self.classificationMask
    ...
    def L2A_CSND_3(self):
        ...
        T1_NDVI = self._T1_NDVI
        T2_NDVI = self._T2_NDVI
        T1_B2T = 0.15
        B02 = self.tables.getBand(Band.BLUE)
        B04 = self.tables.getBand(Band.RED)
        if 'LANDSAT' in self.config.spacecraftName:
    ...
    def L2A_CSND_4(self):
        ...
        T1_R_B8A_B03 = self._T1_R_B8A_B03
        T2_R_B8A_B03 = self._T2_R_B8A_B03
        B03 = self.tables.getBand(Band.GREEN)
        if 'LANDSAT' in self.config.spacecraftName:
            B8A = self.tables.getBand(Band.NEAR_INFRARED)
        else:
    ...
    def L2A_CSND_5_1(self):
        ...
        T11_B02 = self._T11_B02 # -0.40
        T12_B02 = self._T12_B02 #  0.46
        T11_R_B02_B11 = self._T11_R_B02_B11 # 0.55 # 0.70
        T12_R_B02_B11 = self._T12_R_B02_B11 # 0.80 # 1.0
        B02 = self.tables.getBand(Band.BLUE)
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
    ...
    def L2A_CSND_5_2(self):
        ...
        T21_B12 = self._T21_B12 # 0.1
        T22_B12 = self._T22_B12 # -0.09
        T21_R_B02_B11 = self._T21_R_B02_B11 # 2.0
        T22_R_B02_B11 = self._T22_R_B02_B11 # 4.0
        T_B02 = 0.2 # modif JL water TOA reflectance shall be less than 20%
        B02 = self.tables.getBand(Band.BLUE)
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
        B12 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_2)
        if 'LANDSAT' in self.config.spacecraftName:
            B8A = self.tables.getBand(Band.NEAR_INFRARED) # B8A used for additional condition
        else:
            B8A = self.tables.getBand(Band.VEGETATION_4) # B8A used for additional condition
        B04 = self.tables.getBand(Band.RED) # B04 used for additional condition
    ...
    def L2A_CSND_6(self):
        ...
        T1_R_B8A_B11 = self._T1_R_B8A_B11 #0.90
        T2_R_B8A_B11 = self._T2_R_B8A_B11 #1.10
        T1_B02 = -0.25 
        T2_B02 = 0.475
        T_R_B02_B11 = 0.8
        B02 = self.tables.getBand(Band.BLUE)
    ...
    def L2A_CSND_6bis(self):
        ...
        T1_R_B04_B11 = 3.0 #self._T1_R_B04_B11
        T2_R_B04_B11 = 6.0 #self._T2_R_B04_B11
        B04 = self.tables.getBand(Band.RED)
        B11 = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
    ...
    def L2A_CSND_7(self):
        import scipy.misc
        T_CLOUD_LP = self._T_CLOUD_LP
        T_CLOUD_MP = self._T_CLOUD_MP
        T_CLOUD_HP = self._T_CLOUD_HP
        T1_B10 = self._T1_B10
        T2_B10 = self._T2_B10
        B02 = self.tables.getBand(Band.BLUE)
        B10 = self.tables.getBand(Band.CIRRUS)
        ...
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) == True):
            dem = self.tables.getBand(Band.DIGITAL_ELEVATION_MAP)
    ...
    def L2A_SHD(self):
        ...
        # csd2 = csd1 # TODO remove and revert, only to speed up debug
    ...
    def L2A_CSHD_1(self):
        ...
        T_B02_B12 = self._T_B02_B12
        T_water = 6.0

        x = self.config.nrows
        y = self.config.ncols
        BX = zeros((6, x, y), float32)
        BX[0, :, :] = self.tables.getBand(Band.BLUE)
        BX[1, :, :] = self.tables.getBand(Band.GREEN)
        BX[2, :, :] = self.tables.getBand(Band.RED)
        if 'LANDSAT' in self.config.spacecraftName:
            BX[3, :, :] = self.tables.getBand(Band.NEAR_INFRARED)
        else:
            BX[3, :, :] = self.tables.getBand(Band.VEGETATION_4)
        BX[4, :, :] = self.tables.getBand(Band.SHORT_WAVE_INFRARED_1)
        BX[5, :, :] = self.tables.getBand(Band.SHORT_WAVE_INFRARED_2)
        ...
        water = (BX[0,:,:] / clip(BX[4,:,:], 0.0001, 2.0)) > T_water   # identify water pixels with B2/B11 > T_water
        del BX
    ...
    def L2A_CSHD_2(self):
        ...
        # Compute extended y_aa, x_aa dimensions to limit FFT aliasing
        y_aa = int(cloud_mask.shape[0] * 1.5 + 0.5)  # +50% to avoid FFT aliasing
        x_aa = int(cloud_mask.shape[1] * 1.5 + 0.5)  # +50% to avoid FFT aliasing

        # Create filter for convolution (4 cases for the 4 "azimuth" quadrants)
        try:
        ...
        # Define shifts for each quadrant to which solar_azimuth belongs
        shifts = {
            (0, 90): (-ys, xs),
            (90, 180): (ys, xs),
            (180, 270): (ys, -xs),
        ...
        # Find and apply the correct shift values
        for (low, high), (shift_y, shift_x) in shifts.items():
            if low <= solar_azimuth < high:
                filt_b = roll(filt_b, int(shift_y), axis=0)
                filt_b = roll(filt_b, int(shift_x), axis=1)
                break
        ...
        cloud_mask_aa[0:cloud_mask.shape[0], 0:cloud_mask.shape[1]] = copy(cloud_mask[:,:])
        ...
        shadow_prob = copy(shadow_prob_aa[0:cloud_mask.shape[0], 0:cloud_mask.shape[1]])
        del shadow_prob_aa
    ...
    def L2A_WaterPixelRecoveryCCI(self):
        if (self.tables.hasBand(Band.WATER_BODY_INDEX) == True):
            WBI = self.tables.getBand(Band.WATER_BODY_INDEX)
            CM = self.classificationMask
    ...
    def L2A_WaterPixelCleaningwithDEM(self):
        ...
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) == True):
            slope = self.tables.getBand(Band.SLOPE)
            shadow = self.tables.getBand(Band.SHADOW_MAP)
            T_Shadow = 128
            T_Slope = 15
...
#        return 
    ...
    def L2A_TopographicShadowwithDEM(self):
        # ProcessLandsat potential topographic shadow over snow/mountainous area and discard water areas from ESA CCI water bodies
        if (self.tables.hasBand(Band.DIGITAL_ELEVATION_MAP) == True) & (self.tables.hasBand(Band.WATER_BODY_INDEX) == True):
    ...
    def L2A_LandPixelRecoveryB10B09B8A(self):
        if (self.tables.hasBand(Band.LAND_COVERAGE_MAP) and (self.tables.hasBand(Band.WATER_VAPOUR_INPUT)) == True):
            LCM = self.tables.getBand(Band.LAND_COVERAGE_MAP)
            CM = self.classificationMask
            B10 = self.tables.getBand(Band.CIRRUS)
            if 'LANDSAT' in self.config.spacecraftName:
                B8A = self.tables.getBand(Band.NEAR_INFRARED)
                ...
                B8A = self.tables.getBand(Band.VEGETATION_4)
            B09 = self.tables.getBand(Band.WATER_VAPOUR_INPUT)
    ...
    def L2A_CirrusPixelRecoveryB10(self):
        if (self.tables.hasBand(Band.LAND_COVERAGE_MAP) == True):
            LCM = self.tables.getBand(Band.LAND_COVERAGE_MAP)
            CM = self.classificationMask
            B10 = self.tables.getBand(Band.CIRRUS)
    ...
    def process(self):
        from L2A_AuxImport_Landsat import AuxImport #from merge
        self.config.timestamp('Pre process   ')
        self.config.timestamp('Scene Classification Classic   ')
