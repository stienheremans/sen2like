...
class BandIO(object):
    def __init__(self, config):
        ...
        if(self._resolution == 10):
            ...
            self._bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED,
                           Band.VEGETATION_1, Band.VEGETATION_2, Band.VEGETATION_3, Band.VEGETATION_4,
                           Band.WATER_VAPOUR_INPUT, Band.CIRRUS, Band.SHORT_WAVE_INFRARED_1, Band.SHORT_WAVE_INFRARED_2]
            bandDir = 'R20m'
        elif(self._resolution == 30):
            self._bandIndex = [0, 1, 2, 3, 4, 5, 6, 8, 9, 10, 11, 12]
            self._bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED,
                           Band.VEGETATION_1, Band.VEGETATION_2, Band.VEGETATION_3, Band.VEGETATION_4,
                           Band.WATER_VAPOUR_INPUT, Band.CIRRUS, Band.SHORT_WAVE_INFRARED_1, Band.SHORT_WAVE_INFRARED_2]
        ...
        try:
            ...
            chmod_recursive(L2A_QI_DATA, 0o755)
        ...
        S2A_L1C_mask_safe_standard = 'S2?_OPER_MSI_L1?_TL*'
        ...
        if self._firstInit:
            L1C_Tile_PVI_File = os.path.join (self._L2A_QualityDataDir, L2A_TILE_ID + '_PVI.jp2')
            if os.path.exists(L1C_Tile_PVI_File):
                ...
                L1C_Tile_PVI_File = os.path.join(self._L2A_QualityDataDir, L2A_TILE_ID + '_PVI.tif')
                if os.path.exists(L1C_Tile_PVI_File):
                    os.remove(L1C_Tile_PVI_File)
            self._L2A_Tile_PVI_File = os.path.join (self._L2A_QualityDataDir, L2A_TILE_ID + '_PVI' + self._L2A_ImageExtention)
        ...
        self._band_to_code = {
            Band.COASTAL_AEROSOL: 'B01',
            Band.BLUE: 'B02',
            Band.GREEN: 'B03',
            Band.RED: 'B04',
            Band.VEGETATION_1: 'B05',
            Band.VEGETATION_2: 'B06',
            Band.VEGETATION_3: 'B07',
            Band.NEAR_INFRARED: 'B08',
            Band.VEGETATION_4: 'B8A',
            Band.WATER_VAPOUR_INPUT: 'B09',
            Band.CIRRUS: 'B10',
            Band.SHORT_WAVE_INFRARED_1: 'B11',
            Band.SHORT_WAVE_INFRARED_2: 'B12'
        }

        self._band_to_index = {
            Band.COASTAL_AEROSOL: 0,
            Band.BLUE: 1,
            Band.GREEN: 2,
            Band.RED: 3,
            Band.VEGETATION_1: 4,
            Band.VEGETATION_2: 5,
            Band.VEGETATION_3: 6,
            Band.NEAR_INFRARED: 7,
            Band.VEGETATION_4: 8,
            Band.WATER_VAPOUR_INPUT: 9,
            Band.CIRRUS: 10,
            Band.SHORT_WAVE_INFRARED_1: 11,
            Band.SHORT_WAVE_INFRARED_2: 12
        }
    ...
    def get_code_to_band(self, band):
            return self._code_to_band[band]

    def get_band_code(self, band):
        if band.is_msi():
            return self._band_to_code[band]
        else:
            return band.value
    ...
    bands = property(get_bands, set_bands, del_bands, "bands docstring")
    ...
    def checkBandCount(self):
        ...
        for i in bandIndex:
            for filename in dirs:
                ...
                filemask_tif_2 = '*_%3s.TIF' % bandName
                if not fnmatch.fnmatch(filename, filemask_jp2) and not fnmatch.fnmatch(filename, filemask_tif) and not fnmatch.fnmatch(filename, filemask_tif_2):
                    continue
                bandCount += 1
                break
    ...
    def create_lost_band_list(self):
        ...
        while True:
            try:
                ...
                # break
                self.config.timestamp('L2A_Tables: *Check TECQUA for data loss* string not found.')
                return None
    ...
    def read_qlt_msk_jp2(self, image_filepath):
        # read JPEG2000 image into numpy array
        ds = gdal.Open(image_filepath, gdal.GA_ReadOnly)
    ...
    def create_msi_lost_mask(self):
        ...
        quality_mask = zeros((resolution_check, resolution_check)).astype(uint8)
        ...
        for i in range(0, len(Quality)):
            ...
            if os.path.exists(file_path):
                ...
                try:
                    quality_dataset = self.read_qlt_msk_jp2(file_path)  # reading with gdal
                except:
                ...
                self.config.timestamp("L2A_Tables: NO MSK_QUALIT for band {}".format(Quality[i]))
                continue
        ...
        self.setBand(Band.QUALITY_MASK, quality_mask) #self.QMS
        return
    ...
    def importBandList(self):
        ...
        if self._resolution == 10:
            ...
            relevant_bands = [Band.COASTAL_AEROSOL, Band.VEGETATION_1, Band.VEGETATION_2, Band.VEGETATION_3,
                              Band.VEGETATION_4, Band.WATER_VAPOUR_INPUT, Band.CIRRUS,
                              Band.SHORT_WAVE_INFRARED_1, Band.SHORT_WAVE_INFRARED_2]
            for band in relevant_bands:
                if self.hasBand(band):
                    self.removeBandImg(band)
        ...
        for band in bands:
            for filename in dirs:
                bandName = self.get_band_code(band)
                filemask = '*_%3s.*' % bandName
                if not fnmatch.fnmatch(filename, filemask):
                    continue
                res = self.importBandImg(band, os.path.join(sourceDir, filename))
                if not res:
                    return False
                if(bandName == 'B02'):
        ...
        if(self._resolution == 10):
            ...
            srcResolution = '_20m'
            channels = [Band.SCENE_CLASSIFICATION, Band.AEROSOL_OPTICAL_THICKNESS,
                        Band.WATER_VAPOUR, Band.VISIBILITY]
        ...
        if upsampling:
            ...
            for band in channels:
                for filename in dirs:
                    ...
                    if not fnmatch.fnmatch(filename, filemask):
                        continue
                    res = self.importBandRes(band, os.path.join(sourceDir, filename))
                    if bandName == 'VIS':
                        try:
        ...
        if self.config.resolution != 10 :
            if not self.config.Hyper:
                if (self.get_visibility_from_AuxData() == False):
                    ...
                    self.config.timestamp("L2A_Tables: Will try  with local daily-monthly information")
                    self.visibility_extra = True
            else:
                vis_output = 0
                self.visibility_extra = True
        ...
        aux_import = AuxImport(self.config)
        self.dem = False
        self.dem_error_type = 'False' #kept this definition for the old function here
        aux_import.dem_error_type = 'False'
        aux_import._input_dem_resolution = 0.0 # self._input_dem_resolution = 0.0
        ...
        if demDir == 'NONE':
            ...
            if self.config.Hyper:
                if self.visibility_extra:
                    self.config.timestamp("L2A_Tables: trying VIS  with local daily-monthly information")
                    try:
                        ...
                        if vis_output == 1:
                            ...
                            if (self.gdalCAMS_aod550() == True):
                                ...
                                self.config.timestamp(
                                    "L2A_Tables: Continue with local CAMS ECMWF aerosol optical thickness @ 550nm information")
                    except:
                        # Continue without CAMS ECMWF aerosol forecast information
                        self.config.timestamp("L2A_Tables: Continue without CAMS ECMWF aerosol forecast information")

            return True

        # check if DEM is a DTED type, these files must exist in the given directory:
        if self.isDted(): #self.isDted(self.config)
            # yes it is, run dem preparation for DTED:
            demfile, dem_band = aux_import.gdalDEM_dted() # self.gdalDEM_dted()
            if demfile == False:
                ...
                self.importBandRes(Band.DIGITAL_ELEVATION_MAP, dem_band)
                self._input_dem_resolution = aux_import._input_dem_resolution
            ...
            if 'srtm' in (self.config.demReference).lower(): # run DEM preparation for SRTM
                demfile, dem_band = aux_import.gdalDEM_srtm() #self.gdalDEM_srtm()
            elif 'aws' in (self.config.demReference).lower(): # run DEM preparation for AWS Copernicus
                demfile,  dem_band = aux_import.gdalDEM_aws()
            else : # run DEM preparation for Copernicus
                demfile, dem_band = aux_import.gdalDEM_copernicus() # self.gdalDEM_copernicus()
            ...
            if not dem: # continue with flat surface ...
                ...
                elif 'aws' in (self.config.demReference).lower():
                    self.importBandRes(Band.DIGITAL_ELEVATION_MAP, dem_band)
                    self._input_dem_resolution = aux_import._input_dem_resolution
                    if self._input_dem_resolution == 0.00027778:
        ...
        if (demfile != False) & (demfile !='NOT_AVAILABLE'):
            ...
            sdwfile = self.gdalDEM_Shade_Cast(demfile) #aux_import.gdalDEM_Shade_Cast(demfile)
            try:
                self.importBandRes(Band.SHADOW_MAP, sdwfile)
                os.remove(sdwfile)
            except:
            ...
            try:
                self.importBandRes(Band.SLOPE, slpfile)
                os.remove(slpfile)
            except:
            ...
            if self.config.resolution > 10:
                ...
                try:
                    self.importBandRes(Band.ASPECT, aspfile)
                    os.remove(aspfile)
                except:
        ...
        if(self._resolution == 10):
            ...
            band = aux_import.gdalCCI_wb()
            try:
                self.importBandRes(Band.WATER_BODY_INDEX, band)
            except:
            ...
            band = aux_import.gdalCCI_lccs()
            try:
                self.importBandRes(Band.LAND_COVERAGE_MAP, band)
            except:
            ...
            band = aux_import.gdalCCI_snowc()
            try:
                self.importBandRes(Band.SNOW_CONDITION_MAP, band)
            except:
            ...
            # band = aux_import.gdalCAMS_aod550()
            # try:
            #     self.importBandRes(Band.VISIBILITY_INDEX_MAP, band)
            # except:
            #     # Continue without CAMS ECMWF aerosol forecast information
    ...
    def setCornerCoordinates(self):
        ...
        if self.config.ROI == 'AUTO':
            ...
            if self.config.resolution == 60:
                ...
                right = 1830
                scale = float32(1. / 6.)
            elif self.config.resolution == 30: #ROI_Hyper
                bottom = 3660
                right = 3660
                scale = float32(1. / 3.)
    ...
    def computing_visibility_from_auxdata(self, warped_geopotential, warped_aod):
        ...
        vis_from_aux = clip(vis_from_aux, 0,
                            650)  # clipping to avoid integer overflow (To be investigated for lower bound visibility)
    ...
    def get_visibility_from_AuxData(self):
        ...
        # self.importBandRes(Band.VISIBILITY_INDEX_MAP, vis)
        self.setBand(Band.VISIBILITY_INDEX_MAP, vis)
    ...
    def getCAMS_findnc(self, hour1900):
        ...
        print("\n" + "="*50)
        print(f"DEBUG PAD-CHECK: {ncfile}")
        print(f"BESTAAT DIT PAD? {os.path.exists(ncfile)}")
        print("="*50 + "\n")
    ...
    def gdalCAMS_aod550(self):
        ...
        self.importBandRes(Band.VISIBILITY_INDEX_MAP, cams_vis) #self.importBandRes(self.VIM, cams_vis) Vself.tables.hasBand(Band.VISIBILITY_INDEX_MAP)
    ...
    def gdalDEM_srtm(self):
        ...
        self.importBandRes(Band.DIGITAL_ELEVATION_MAP, srtmf_dst_int16)  # fix for SIIMPC-792, JL, srtmf_dst_int16 #self.DEM
        if isTemporary:
            os.remove(srtmf_src)
        # fix for SIIMPC-792, JL
    ...
    def gdalDEM_Shade_Cast(self, demfile):
        ...
        if topo_shadows_gdal.sum() > 0:  # compute casted shadow only if at least one pixel is not illluminated by the sun
            ...
            try:
                dem = self.getBand(Band.DIGITAL_ELEVATION_MAP)
            except Exception as e:
                self.logger.fatal(e, exc_info=True)
            ...
            if self._resolution > 10:
                ...
                resolution = 20
                dem = (skit_resize(dem.astype(int16), ([cols/2, rows/2]), order=1) * 32767.).round().astype(int16)
            ...
            sdw = topographicshadows_cython_03.project_shadows(dem, array([sza, saa]), float64(resolution), float64(resolution))
        ...
        # self.importBandRes(Band.SHADOW_MAP, sdwfile) #(self.SDW, sdwfile)
        # os.remove(sdwfile)
        return sdwfile
    ...
    def gdalDEM_Slope(self, demfile):
        ...
        self.importBandRes(Band.SLOPE, slpfile) #(self.SLP, slpfile)
        os.remove(slpfile)
        return True
    ...
    def gdalDEM_Aspect(self, demfile):
        ...
        self.importBandRes(Band.ASPECT, aspfile) #(self.ASP, aspfile)
        os.remove(aspfile)
        return True

    def importBandImg(self, band, filename):
        h5file = None
        bandName = self.get_band_code(band)
        if self.hasBand(band):
        ...
        if  self.config.Hyper:
            ...
            self.hyper_projection = dataset.GetProjection()
            self.hyper_geotransform = dataset.GetGeoTransform()
            indataset = dataset.GetRasterBand(1).ReadAsArray()
            if bandName == 'B02':
                if 'TIF' in filename:
                    jp2_filename = filename.replace('TIF','jp2')
                else:
                    jp2_filename = filename.replace('tif', 'jp2')
                kwarg = [' -of JP2OpenJPEG']
                gdal.Translate(jp2_filename, filename, options=kwarg)
                ...
                try: # to be compatible with OpenJPEG < 2.3:
                    ...
                    pass
                indataset_for_geobox = glymur.Jp2k(jp2_filename)
                self.config.set_geobox(self.getAssociationBox(indataset_for_geobox.box), 30)
                try:
                    os.remove(jp2_filename)
                    os.remove(jp2_filename + '.aux.xml')
                except:
                    pass
            dataset = None
        ...
        if band.is_msi(): # exclude DEM related Bands: # if index < 13:
            if self.config.ROI == 'AUTO':
        ...
        if self.config.productVersion > float32(14.6):
            index_offset=self._band_to_index[band]
            offset = self.config.radio_add_offset_list[index_offset] #index
    ...
    def importBandRes(self, band, filename):
        h5file = None
        bandName = self.get_band_code(band)
        relevant_bands = [Band.SCENE_CLASSIFICATION, Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR, Band.VISIBILITY]
        if ((band in relevant_bands) and (self.config.resolution == 10)) and self.hasBand(band):
            # resample SCL, AOT, WVP, VIS and DEM related bands:
            self.config.timestamp('L2A_Tables: band ' + bandName + ' needs to be resampled')
            indataset = self.resampleBand(band, self.getBand(band))
        ...
        if band.is_msi() and self.config.ROI != 'OFF': # exclude DEM related Bands:
            top = 0
            left = 0
            bottom = src_nrows
            right = src_ncols
    ...
    def resampleBand(self, band, indataArr):
        src_nrows = indataArr.shape[0]
        ...
        if (src_nrows / tgt_nrows) == 2:
            ...
            if band == Band.SCENE_CLASSIFICATION or\
                band == Band.AEROSOL_OPTICAL_THICKNESS or\
                band == Band.WATER_VAPOUR or\
                band == Band.VISIBILITY:
                # order=0 is for nearest neighbor (SCL, AOT, WVP, VIS):
                return skit_resize(indataArr.astype(uint16), ([size_up_nrows, size_up_ncols]), order=0, preserve_range=True).astype(uint16)
            elif band == Band.CIRRUS or band == Band.WATER_VAPOUR_INPUT:
                # order=1 is for bi-linear interpolation (B10 upsampling from 60 m to 20 m):
                return skit_resize(indataArr.astype(uint16), ([size_up_nrows, size_up_ncols]), order=1, preserve_range=True).astype(uint16)
            else:
                # order=3 is for bi-cubic spline (other bands):
                return skit_resize(indataArr.astype(uint16), ([size_up_nrows, size_up_ncols]), order=3, preserve_range=True).astype(uint16)
        return
    ...
    def downsampleBandList_20to60_andExport(self):
        ...
        if self.config.productVersion < 14:
            ...
            bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2,
                     Band.VEGETATION_3, Band.VEGETATION_4, Band.WATER_VAPOUR_INPUT, Band.SHORT_WAVE_INFRARED_1,
                     Band.SHORT_WAVE_INFRARED_2, Band.DIGITAL_ELEVATION_MAP, Band.SCENE_CLASSIFICATION,
                     Band.SNOW_MAP, Band.CLOUD_MAP, Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR,
                     Band.DARK_DENSE_VEGETATION]
            ...
            bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2,
                     ...
                     Band.SHORT_WAVE_INFRARED_2, Band.DIGITAL_ELEVATION_MAP, Band.SCENE_CLASSIFICATION,
                     Band.SNOW_MAP, Band.CLOUD_MAP, Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR,
                     Band.DARK_DENSE_VEGETATION, Band.TRUE_COLOR_IMAGE]
        else: # > 14.6:
            bandIndex = [0, 1, 2, 3, 4, 5, 6, 8, 9, 11, 12, 13, 14, 15, 16, 17, 18, 27, 33]
            bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2,
                     Band.VEGETATION_3, Band.VEGETATION_4, Band.WATER_VAPOUR_INPUT, Band.SHORT_WAVE_INFRARED_1,
        ...
        RGB = [Band.RED, Band.GREEN, Band.BLUE]  # [3, 2, 1] ['B04', 'B03', 'B02']
        ...
        for b in bands:
            if self.config.scOnly or not b.is_msi():
                if b == Band.DIGITAL_ELEVATION_MAP and not self.config.demOutput:
                    continue
                elif b == Band.DARK_DENSE_VEGETATION and not self.config.ddvOutput:
                    continue
                elif b == Band.TRUE_COLOR_IMAGE and not self.config.tciOutput:
                    continue
                indataset = self.getBand(b)
                if b.is_msi(): # for SC_ONLY
                    indataset *= self.config.dnScale
                    if self.config.productVersion > float32(14.6):
                        band_temp_name = b
                        index_offset = self._band_to_index[band_temp_name]  # FCP
                        offset = self.config.radio_add_offset_list[index_offset] * -1
                        indataset[indataset != 0] += uint16(offset)
                        indataset[indataset < 0] = 1
            else:
                indataset = self.getTmpBand(b)
            try:
                bandName = self.get_band_code(b)
                filename = self._L2A_Tile_BND_File

                if b == Band.SNOW_MAP:
                    filename = self._L2A_Tile_SNW_File
                elif b == Band.CLOUD_MAP:
                    filename = self._L2A_Tile_CLD_File
                elif b == Band.AEROSOL_OPTICAL_THICKNESS:
                    filename = self._L2A_Tile_AOT_File
                elif b == Band.WATER_VAPOUR:
                    filename = self._L2A_Tile_WVP_File
                elif b == Band.TRUE_COLOR_IMAGE:
                    if (self.config.productVersion < 14) or (self.config.tciOutput == False):
                        continue
                    filename = self._L2A_Tile_TCI_File
                elif b == Band.DIGITAL_ELEVATION_MAP:
                    ...
                    mask = self.getBand(Band.SCENE_CLASSIFICATION)
                    indataset[mask != self.config.noData] += 10000
                    indataset[mask == self.config.noData] = 0
                    indataset = indataset.astype(uint16)
                    del mask
                elif b == Band.DARK_DENSE_VEGETATION:
                    if (self.config.ddvOutput == False):
                        continue
                    filename = self._L2A_Tile_DDV_File
                # special treatment for scene class:
                if b == Band.SCENE_CLASSIFICATION:
                    filename = self._L2A_Tile_SCL_File
                    rows = uint16(float(self.config.nrows)/3.0+0.5)
                    cols = uint16(float(self.config.ncols)/3.0+0.5)
                    indataset = median_filter(indataset, 3)
                    indataset = (skit_resize(indataset.astype(uint8), ([rows, cols]), order=0, mode='constant') * 255.).round().astype(uint8)
                elif b != Band.TRUE_COLOR_IMAGE: # any other band except SCL or TCI:
                    indataset = uint16(block_reduce(indataset, block_size=(3, 3), func=mean) + 0.5)
                ...
                if not self.config.scOnly:
                    if self.config.ROI != 'OFF' and not b in RGB:
                        outframe[rowTop:rowBottom, colLeft:colRight] = indataset
                        indataset = outframe
                else:
                    if self.config.ROI != 'OFF':
                        outframe[rowTop:rowBottom, colLeft:colRight] = indataset
                ...
                if b != Band.TRUE_COLOR_IMAGE:
                    if self.config.raw:
                        self.exportRawImage(filename, indataset)
                    elif self.config.tif:
                        self.exportTifImage(filename, indataset)
                    elif b == Band.SCENE_CLASSIFICATION and self.config.scCog:
                        filename_tif = os.path.splitext(filename)[0] + '.tif'
                        self.glymurWrapper(filename, indataset)
                ...
                if (b != Band.VISIBILITY and not consolidatedTile and self.config.operationMode == 'TOOLBOX'):
                    ifn = 'IMAGE_FILE'
                    imageFile2a = etree.Element(ifn)
                    # by intention os.path.join is not used here, as otherwise validation on windows fails:
                    if (b == Band.CLOUD_MAP or b == Band.SNOW_MAP or b == Band.DARK_DENSE_VEGETATION):
                        continue
                    elif b == Band.DIGITAL_ELEVATION_MAP:
    ...
    def exportBandList(self):
        ...
        if(self._resolution == 10):
            if self.config.productVersion < 14:
                ...
                bands = [Band.BLUE, Band.GREEN, Band.RED, Band.NEAR_INFRARED, Band.DIGITAL_ELEVATION_MAP,
                         Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR]
            else:
                bandIndex = [1, 2, 3, 7, 13, 17, 18, 33]
                bands = [Band.BLUE, Band.GREEN, Band.RED, Band.NEAR_INFRARED, Band.DIGITAL_ELEVATION_MAP,
                         Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR, Band.TRUE_COLOR_IMAGE]
        elif(self._resolution == 20):
            # SIIMPC-1167: implementation of version dependency of output bands:
            if self.config.productVersion < 14:
                bandIndex = [1, 2, 3, 4, 5, 6, 8, 11, 12, 13, 14, 15, 16, 17, 18, 19, 27]
                bands = [Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2, Band.VEGETATION_3,
                         Band.VEGETATION_4, Band.SHORT_WAVE_INFRARED_1, Band.SHORT_WAVE_INFRARED_2,
                         Band.DIGITAL_ELEVATION_MAP, Band.SCENE_CLASSIFICATION, Band.SNOW_MAP, Band.CLOUD_MAP,
                         Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR, Band.VISIBILITY,
                         Band.DARK_DENSE_VEGETATION]
            elif self.config.productVersion <= float32(14.6):
                ...
                bands = [Band.COASTAL_AEROSOL, Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2,
                         Band.VEGETATION_3, Band.VEGETATION_4, Band.SHORT_WAVE_INFRARED_1,
                         Band.SHORT_WAVE_INFRARED_2, Band.DIGITAL_ELEVATION_MAP, Band.SCENE_CLASSIFICATION,
                         Band.SNOW_MAP, Band.CLOUD_MAP, Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR,
                         Band.VISIBILITY, Band.DARK_DENSE_VEGETATION]
        elif(self._resolution == 30):
            # SIIMPC-1167: implementation of version dependency of output bands:
            if self.config.productVersion < 14:
                bandIndex = [1, 2, 3, 4, 5, 6, 8, 11, 12, 13, 14, 15, 16, 17, 18, 19, 27]
                ...
                bandIndex = [0, 1, 2, 3, 4, 5, 6, 8, 11, 12, 13, 14, 15, 16, 17, 18, 19, 27, 33]
                bands = [Band.COASTAL_AEROSOL,Band.BLUE, Band.GREEN, Band.RED, Band.VEGETATION_1, Band.VEGETATION_2, Band.VEGETATION_3,
                         Band.VEGETATION_4, Band.SHORT_WAVE_INFRARED_1, Band.SHORT_WAVE_INFRARED_2,
                         Band.DIGITAL_ELEVATION_MAP, Band.SCENE_CLASSIFICATION, Band.SNOW_MAP, Band.CLOUD_MAP,
                         Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR, Band.VISIBILITY,
                         Band.DARK_DENSE_VEGETATION, Band.TRUE_COLOR_IMAGE]
        ...
        RGB = ['B04', 'B03', 'B02'] #[3, 2, 1] # [3, 2, 1] [Band.RED, Band.GREEN, Band.BLUE]

        if self.config.scOnly:
            for band in bands:
                if  band.is_msi():
                    indataset = uint16(self.getBand(band) * self.config.dnScale)
                    self.setTmpBand(band, indataset)
                elif band in [Band.AEROSOL_OPTICAL_THICKNESS, Band.WATER_VAPOUR,
                               Band.VISIBILITY, Band.DARK_DENSE_VEGETATION]:
                    indataset = zeros([self.config.nrows, self.config.ncols], dtype=uint16)
                    self.setBand(band, indataset)
        if self.config.track_iwaterwv == 0:
                band_empty = zeros([self.config.nrows, self.config.ncols], dtype=uint16)
                self.setBand(Band.WATER_VAPOUR, band_empty)
        ...
        if self.config.ROI == 'AUTO':
            ...
            if self.config.resolution == 60:
                ...
                outframe = zeros([3660, 3660], dtype=uint16)
                scale = 1.0 / 3.0 #to be checked
            elif self.config.resolution == 10:
        ...
        try:
            ...
            for band in bands:
                ...
                if band.is_msi():
                    ...
                    node = h5file.get_node('/arrays', bandName)
                elif bandName == 'VIM' and not self.hasBand(band):
                ...
                if (self._resolution == 60):
                    ...
                    filename = filename.replace('20m', '60m')
                if bandName != 'TCI':
                    band_temp_name = band #FCP  warning than band changes meaning
                    band = node.read()
                    # here to add the offset again:
                    if self.config.productVersion > float32(14.6) and band_temp_name.is_msi(): #FCP
                        # offset is negative:
                        if self.config.scOnly: #FCP
                            index_offset = self._band_to_index[band_temp_name]  # FCP
                            offset = self.config.radio_add_offset_list[index_offset] * -1
                            band[band != 0] += uint16(offset)
                            band[band < 0] = 1
                    ...
                    if bandName in RGB:
                        h5file.close()
                        temp_band_index=self.get_code_to_band(bandName)
                        self.setTmpBand(temp_band_index, band)
                        h5file.close()
                        h5file = open_file(self._resdb, mode='r')
                    if self.config.raw:
...
#                        os.remove(filename)
    ...
    def glymurWrapper(self, filename, band):
        ...
        if 'PVI' in filename:
            ...
            kwargs = {"cbsize": (4, 4), "tilesize": (192, 192), "prog": "LRCP", "psizes": ((64, 64), (64, 64), (64, 64), (64, 64), (64, 64))}
        elif self._resolution == 30:
            kwargs = {"cbsize": (8, 8), "tilesize": (512, 512), "prog": "LRCP", "psizes": ((128, 128), (128, 128), (128, 128), (128, 128), (128, 128))}
        elif self._resolution == 20:
            kwargs = {"cbsize": (8, 8), "tilesize": (640, 640), "prog": "LRCP", "psizes": ((128, 128), (128, 128), (128, 128), (128, 128), (128, 128))}
        elif self._resolution == 10:
            kwargs = {"cbsize": (64, 64), "tilesize": (1024, 1024), "prog": "LRCP", "psizes": ((256, 256), (256, 256), (256, 256), (256, 256), (256, 256))}
    ...
    def exportTifImage(self, filename, band):
        ...
        if self.config.Hyper:
            projection = self.hyper_projection
            geotransform = self.hyper_geotransform
        else:
        ...
        if isTCI or isPVI:
            ...
            if isPVI:
                geotransform = (geotransform[0], float(320.0), geotransform[2], geotransform[3], geotransform[4], float(-320.0))
        ...
        if isSCL and self.config.scCog:
            ...
            MEM_driver = gdal.GetDriverByName("MEM")
            MEM_dataset = MEM_driver.CreateCopy("", srcfile)
            if band.shape[0] == 3660:
                MEM_dataset.BuildOverviews(resampling="MODE", overviewlist=(2, 4, 12)) #fix here
    ...
    def exportJp2Image(self, filename, band):
        ...
        ds = driver.Create('', xsize=w, ysize=h, bands=numBands, eType=pixelFormat)
        # ds = driver.Create(filename, w, h, numBands, pixelFormat, options=creation_options)
        ds.SetGeoTransform(geotransform)
        ds.SetProjection(projection)
        ...
        driver_JP2 = gdal.GetDriverByName('JP2OpenJPEG')
        ...
        if isPVI:
            ...
            if self.config.resolution == 60:
                creation_options.update({
                    ...
                    'BLOCKYSIZE': 192,
                    'PROGRESSION': 'LRCP',
                    'PRECINCTS': '{64,64},{64,64},{64,64},{64,64},{64,64}',
                })
            elif self.config.resolution == 20:
                creation_options.update({
                'CODEBLOCK_WIDTH': 8,
                ...
                'BLOCKYSIZE': 640,
                'PROGRESSION': 'LRCP',
                'PRECINCTS': '{128,128},{128,128},{128,128},{128,128},{128,128}',
                })
            elif self.config.resolution == 10:
                creation_options.update({
                'CODEBLOCK_WIDTH': 64,
                ...
                'BLOCKYSIZE': 1024,
                'PROGRESSION': 'LRCP',
                'PRECINCTS': '{256,256},{256,256},{256,256},{256,256},{256,256}',
                })

        creation_options = list(map(lambda ops: ops[0] + '=' + str(ops[1]), creation_options.items()))
        data_set2 = driver_JP2.CreateCopy(filename, ds, options=creation_options)
    ...
    def createRgbImages(self):
        ...
        if self._firstInit:
            ...
            try:
                Band_T_B04, Band_T_B03, Band_T_B02 = self.getTmpBand(Band.RED), self.getTmpBand(Band.GREEN), self.getTmpBand(Band.BLUE)

                if self.config.productVersion > float32(14.6):
                    Band_T_B04[(Band_T_B04 <= 1000) & (Band_T_B04 != 0)] = 1001
                    Band_T_B03[(Band_T_B03 <= 1000) & (Band_T_B03 != 0)] = 1001
                    Band_T_B02[(Band_T_B02 <= 1000) & (Band_T_B02 != 0)] = 1001
                ...
                Band_B04, Band_B03, Band_B02 = self.getBand(Band.RED), self.getBand(Band.GREEN), self.getBand(Band.BLUE) #self.B04 .B03 .B02

                if self.config.productVersion > float32(14.6):
                    Band_B04[(Band_B04 <= 1000) & (Band_B04 != 0)] = 1001
                    Band_B03[(Band_B03 <= 1000) & (Band_B03 != 0)] = 1001
                    Band_B02[(Band_B02 <= 1000) & (Band_B02 != 0)] = 1001
            ...
            try:
                ...
                if self.config.tif:
                    ...
                    if not self.config.Hyper:
                        ...
                        self.exportJp2Image(pvi, dstack((r, g, b)))
                # end fix for SIIMPC-558.3
        ...
        if (self.config.productVersion >= 14) and (self.config.tciOutput == True):
            ...
            try:
                ...
                Band_B04, Band_B03, Band_B02 = self.getBand(Band.RED), self.getBand(Band.GREEN), self.getBand(Band.BLUE)
    ...
    def hasBand(self, band):
        ...
        try:
            ...
            self.logger.debug('Channel %s is present',  bandName)
            return True
        except:
            self.logger.debug('Channel %s is not available', bandName)
            return False
        finally:
            if h5file:
                h5file.close()
    ...
    def getBand(self, band, radiance=False): #getBand(self, index):
        ...
        # if method is called with integer bands are 0-indexed x: 0 = band 9 .Band.Cirrus
        h5file = None
        try:
            bandName = self.get_band_code(band)
        except:
            band = self._bands[band]
            bandName = self.get_band_code(band)
        if radiance:
            index = self.reindex(band)
        try:
            if band.is_msi():
                h5file = open_file(self._imgdb, mode='r')
                node = h5file.get_node('/arrays', bandName)
                ...
                if src_nrows == tgt_nrows:
                    array = float32(array)
                    array = (array / float32(self.config.dnScale))  # scaling from 0:1
                    if radiance: #form merge_
                        return array * self.config.e0[index] * cos(radians(self.config.solze)) / pi
                    else:
                        return array #form merge_
            # else:
            resArr = self.getResampledBand(band)
            try:
                if resArr == False:
                    self.config.timestamp('L2A_Tables: band ' + bandName + ' must be resampled')
                    array = self.resampleBand(band, array)
                    if not self.setResampledBand(band, array):
                        return False
            except:
                array = resArr
            if not band.is_msi():
    ...
    def setBand(self, band, array):
        ...
        try:
            ...
            self.logger.debug('Channel %s added to table', band.name)
            node.append(array)
    ...
    def removeBandImg(self, band):
        ...
        try:
            ...
            if(h5file.__contains__('/arrays/' + bandName)):
                ...
                node.remove()
                table.flush()
                self.logger.debug('Channel %s removed from table', band.name)
            return True
        except:
            return False
        finally:
    ...
    def setTmpBand(self, band, array):
        h5file = None
        try:
            bandName = self.get_band_code(band)
        except:
        ...
        try:
            ...
            self.logger.debug('Temporary channel ' + bandName + ' added to table')
            node.append(array)
            if self.config.logLevel == 'DEBUG':
                self.readoutStatistics(bandName, read = False)
            table.flush()
    ...
    def getResampledBand(self, band):
        h5file = None
        bandName = self.get_band_code(band)
        try:
            h5file = open_file(self._resdb, mode='r')
    ...
    def setResampledBand(self, band, array):
        h5file = None
        bandName = self.get_band_code(band)
        try:
            h5file = open_file(self._resdb, mode='a')
            ...
            self.logger.debug('Resampled band ' + band.name + ' added to table')
            node.append(array)
            if self.config.logLevel == 'DEBUG':
                self.readoutStatistics(bandName, read = False)
            table.flush()
        ...
        if self.config.resolution == 10:
            new_index = \
                ['NA',  # NA
                 ...
                 'NA',  # CIRRUS\
                 'NA',  # SHORT_WAVE_INFRARED_1\
                 'NA',  # SHORT_WAVE_INFRARED_2 \
                 'NA',  # THERMAL_INFRARED_1 \
                 'NA',  # THERMAL_INFRARED_2 \
            ...
            new_index = \
                ['NA',  # NA
                 ...
                 'NA',  # NEAR_INFRARED\
                 7,  # VEGETATION_4\
                 8,  # WATER_VAPOUR_INPUT\
                 9,  # CIRRUS\
                 10,  # SHORT_WAVE_INFRARED_1\
                 11,  # SHORT_WAVE_INFRARED_2 \
                 'NA',  # THERMAL_INFRARED_1 \
                 'NA',  # THERMAL_INFRARED_2 \
                 'NA',  # LONG_WAVE_INFRARED_1\
                 'NA',  # LONG_WAVE_INFRARED_2\
