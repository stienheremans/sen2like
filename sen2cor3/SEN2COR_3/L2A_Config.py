...
def transform_utm_to_wgs84(easting, northing, zone1, zone2):
    ...
    return utm_to_wgs84_geo_transform.TransformPoint(double(easting), double(northing), 0)  # returns lon, lat, altitude
...
def transform_wgs84_to_utm(lon, lat):
    ...
    return wgs84_to_utm_geo_transform.TransformPoint(double(lon), double(lat), 0)  # returns easting, northing, altitude
...
class L2A_Config(object):
    def __init__(self, logger, input_dir=False):
        ...
        if (input_dir):
            ...
            self._L2A_BOA_NEGATIVE_VALUES_LIST = zeros([13], dtype=uint32)
            ...
            # For Tif Export
            self.geotransform_from_jp2_60 = []
            self.projection_from_jp2 = []
            # In case of exporting empty wvp band
            self.track_iwaterwv = 1
            self.default_nrows = None
            self.default_ncols = None
    ...
    Hyper = property(get_Hyper, set_Hyper, del_Hyper, "Hyper_MS' docstring")
    ...
    def init_home_directory(self, operationMode):
        ...
        if not os.path.isfile(self._processingEstimationFn):
            ...
            config.set('time estimation', 't_est_30', self._tEst30)
    ...
    def create_L2A_UserProduct(self):
        ...
        firstInit = False
        if self.namingConvention == 'SAFE_STANDARD':
            L1C_UP_MASK = 'S2?_OPER_PRD_MSIL1?*.SAFE'
        else:
            L1C_UP_MASK = 'S2?_MSIL1?*.SAFE'
        ...
        if self.namingConvention == 'SAFE_STANDARD':
            L1C_DS_MASK = 'S2?_OPER_MSI_L1?_DS_*'
        else:
            L1C_DS_MASK = 'DS_*'
        ...
        if (os.name == 'nt') and (self.namingConvention == 'SAFE_STANDARD'):
            ...
            L1C_UP_DIR_ = '\\'.join(['\\\\?', L1C_UP_DIR])
            L1C_INSPIRE_XML_ = '\\'.join(['\\\\?', L1C_INSPIRE_XML])
            L1C_MANIFEST_SAFE_ = '\\'.join(['\\\\?', L1C_MANIFEST_SAFE])
            L2A_UP_DIR_ = '\\'.join(['\\\\?', L2A_UP_DIR])
            L2A_INSPIRE_XML_ = '\\'.join(['\\\\?', L2A_INSPIRE_XML])
            L2A_MANIFEST_SAFE_ = '\\'.join(['\\\\?', L2A_MANIFEST_SAFE])
        else:
        ...
        if not os.path.exists(os.path.join(L2A_UP_DIR, GRANULE)):
            ...
            try:
                ...
                chmod_recursive(L2A_UP_DIR_, 0o755)
            ...
            # create user product:
            if self.namingConvention == 'SAFE_STANDARD':
                S2A_mask = 'S2?_OPER_MTD_SAFL1?*.xml'
            else:
                S2A_mask = 'MTD_MSIL1?.xml'
            try:
    ...
    def configure_L2A_UP_metadata(self):
        ...
        if self.productVersion >= float32(15.0):
    ...
    def create_L2A_Tile(self):
        ...
        if (os.name == 'nt') and (self.namingConvention == 'SAFE_STANDARD'):
            # special treatment for windows for long pathnames:
            L1C_TILE_ID_ = '\\'.join(['\\\\?', L1C_TILE_ID])
            L2A_TILE_ID_ = '\\'.join(['\\\\?', L2A_TILE_ID])
            configFn = '\\'.join(['\\\\?', self.configFn])
        else:
        ...
        if (os.path.exists(L2A_TILE_ID_)):
            ...
            chmod_recursive(L2A_TILE_ID_, 0o755)
            ...
            L1C_MTD_TL_MASK_STANDARD = 'S2?_OPER_MTD_L1?_TL_*.xml'
    ...
    def setTimeEstimation(self, resolution):
        ...
        try:
            ...
            tEst30 = config.getfloat('time estimation', 't_est_30') * factor
        ...
        if (resolution == 60):
            ...
            self._tEstimation = tEst30
        elif (resolution == 20):
            self._tEstimation = tEst20
        elif (resolution == 10):
            self._tEstimation = (tEst20 + tEst10)
        else:
            self._tEstimation = (tEst60 + tEst30 + tEst20 + tEst10)
        return
    ...
    def writeTimeEstimation(self, tMeasure):
        ...
        try:
            ...
            if (self.resolution == 60):
                ...
                tEst = config.getfloat('time estimation', 't_est_30')
                tMeasureAsString = str((tEst + tMeasure) / 2.0)
                config.set('time estimation', 't_est_30', tMeasureAsString)
    ...
    def readPreferences(self):
        ...
        self.track_iwaterwv = int32(par.pyval)
        ...
        value = par.pyval
        if value == 'TRUE':
            self.cirrus_correction = True #1
            self.timestamp('Warning: Cirrus correction is still under implementation: automatically set to False')
            self.logger.warning('Warning: Cirrus correction is still under implementation: switched to False')
            self.cirrus_correction = False #0
        else:
            self.cirrus_correction = False #0
    ...
    def readTileMetadata(self):
        ...
        for i in range(sizelen):
            if int(size[i].attrib['resolution']) == self._resolution:
                ...
                if self._resolution != 10:
                    self.default_nrows = int(size[i].NROWS)
                    self.default_ncols = int(size[i].NCOLS)
                break
        ...
        if self.ROI == 'AUTO':
            ...
            self.nrows = nrows
            self.ncols = ncols
            xy = self.get_geo_coordinates_sentinel()
        ...
        # night-time acquisition sanity check:
        if self._solze_noclip > 90:
            self.logger.error(
                'This product corresponds to a night-time acquisition: sza = {:.2f}, processing will terminate'.format(
                    self._solze_noclip))
            sys.exit(1)
    ...
    def createAtmDataFilename(self):
        ...
        try:
            ...
            if self._Hyper:
                ...
                self._satelliteId = ('S' + self.spacecraftName[-2:])
            self.logger.info('look up table for %s found and used' % self._satelliteId)
            self.lut_data_filelist.append(atmDataFn)
            return
        except:
            self.logger.warning('no specific look up table for %s found, default one will be used instead' % ('S' + self.spacecraftName[-2:]))
            self.atmDataFn = self.atmDataFn.replace('S2?', 'S2A')
        try:
            os.stat(self.atmDataFn)
    ...
    def assignOzoneContent(self):
        ...
        self.ozoneContent = builtins.min(columns, key=columns.get)
    ...
    def setProductVersion(self):
        ...
        if self.operationMode == 'TOOLBOX':
            ...
            if self.namingConvention == 'SAFE_STANDARD':
                filemask = 'S2?_????_MTD_L1?_DS_*.xml'
            else:
                filemask = 'MTD_DS.xml'
        ...
        if self.operationMode != 'GENERATE_DATASTRIP':
            ...
            if self.namingConvention == 'SAFE_STANDARD':
                filemask = 'S2?_????_MTD_L1?_TL_*.xml'
            else:
                filemask = 'MTD_TL.xml'
            found = False
            for rt, dirs, files in os.walk(path):
        ...
        for rt, dirs, files in os.walk(path):
            ...
            if breaker:
                break
        if not mtd_found:
            self.logger.error('Product metadata file cannot be found.\n')
            return False
        ...
        if (productVersion == int(12)):
            ...
            if PSD != 'DEFAULT':
                ...
                if self._Hyper:
                    ...
                    elif float32(4.00) <= processingBaseline <= float32(5.10):
                        productVersion = float32(14.9)
                    else:
                        self.logger.warn(
                            'This Processing Baseline {:05.2f} built on PSD 14.x is unknown'.format(processingBaseline))
                        self.logger.warn(
                            'Version 14.9 will be used by default, but warnings during validation might occur.\n')
                        productVersion = float32(14.9)
        elif (productVersion == int(15)):
            xp = L2A_XmlParser(self, 'GIPP')
            node = xp.getNode('Common_Section')
            if node is None: self.parNotFound(node)
            PSD = node.PSD_Version.pyval
            if PSD != 'DEFAULT':
                ...
                if processingBaseline == float32(5.10):
                    productVersion = float32(15.0)
                elif processingBaseline == float32(5.11):
                    productVersion = float32(15.0)
                else:
                    self.logger.warn('This Processing Baseline {:05.2f} built on PSD 15.x is unknown'.format(processingBaseline))
                    self.logger.warn('Version 15.0 will be used by default, but warnings during validation might occur.\n')
                    productVersion = float32(15.0)
    ...
    def setTileMetadataReference(self):
        test = os.path.join(self.output_dir, self.tile.replace('L1?', 'L2A'), 'MTD_TL.xml')
        if os.path.isfile(test):
    ...
    def calc_region_of_interest(self):
        ...
        for filename in sorted(glob.glob(os.path.join(filepath, '*_B??.*'))): # .jp2
            ...
            if self._rowTop == None:
                self._rowTop = -1
            if self._colLeft == None:
                self._colLeft = -1
            if self._rowBottom == None:
                self._rowBottom = -1
            if self._colRight == None:
                self._colRight = -1
    ...
    def get_geo_coordinates_sentinel(self):
        ...
        if self.ROI == 'AUTO': # scale according to resolution
            if self.resolution == 60:
                scale = 1
            elif self.resolution == 30: #ROI_Hyper
                scale = 2
            elif self.resolution == 20:
                scale = 3
            elif self.resolution == 10:
    ...
    def get_lon_lat(self, xy):
        ...
        # latMin,lonMin, dummy = transform_utm_to_wgs84(xy[1, 0], xy[1, 1], zone1, zone2)
        # latMax,lonMax, dummy = transform_utm_to_wgs84(xy[3, 0], xy[3, 1], zone1, zone2)
