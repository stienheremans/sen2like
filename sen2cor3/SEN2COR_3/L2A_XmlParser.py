...
class L2A_XmlParser(object):
    def __init__(self, config, product):
        ...
        if (product == 'GIPP'):
            ...
            self._xmlFn = config.L2A_QI_REPORT_XML_LANDSAT
            self._scheme = config.qiReportScheme2a_LANDSAT
        else:
    ...
    def validate(self):
        ...
        """
        # temporarily validate in case of Landsat imagery
        if self._config.spacecraftName == 'LANDSAT_8' or self._config.spacecraftName == 'LANDSAT_9':
            return True
    ...
    def convert(self):
        import codecs, re
        objectify.deannotate(self._root, xsi_nil=True, cleanup_namespaces=True)
        outstr = etree.tostring(self._root, pretty_print=True).decode()
