...
class L2A_Quality(object):
    def __init__(self, config, tables):
        ...
        if config.scOnly == False:
            ...
            if self._config.productVersion < np.float32(15.0):
                self._aot_retrieval_method = 'DEFAULT'
            else:
                self._aot_retrieval_method = 'NA'
        ...
        try:
            ...
            elif self._config.resolution == 30:
                self._nr_px_full_tile = (3660 * 3660)
            elif self._config.resolution == 60:
        ...
        self._solar_zenith_angle =  config.get_solze_noclip() #config.get_solze()
        ...
        if config.demType == 'SRTM':
            ...
            self._dem_type = 'CDSE_COPERNICUS_90'
        elif config.demType == 'COPERNICUS_30':
            self._dem_type = 'CDSE_COPERNICUS_30'
    ...
    def update_qi(self):
        switch_case = {
            ...
            'CAST_SHADOW_PERCENTAGE' : format('%f' % self.get_percentage\
        ...
        if not self._config.Hyper:
            switch_case = {
                ...
                'B11': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[10])),
                'B12': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[11])),
                "LUT_DATA_FILES": self._config.lut_data_filelist
            }
        else: # Different switch case for PRISMA because B08 is not present in the L1C S2P
            switch_case = {
                'AOT_RETRIEVAL_ACCURACY': format('%f' % self._aot_retieval_accuracy),
                'GRANULE_MEAN_AOT': format('%f' % self._granule_mean_aot),
                'WV_RETRIEVAL_ACCURACY': format('%f' % self._water_vapour_retrieval_accuracy),
                ...
                'B06': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[5])),
                'B07': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[6])),
                'B8A': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[7])),
                'B11': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[10])),
                'B12': format('%f' % self.get_percentage(self._negative_boa_pixels, self._config.L2A_BOA_NEGATIVE_VALUES_LIST[11])),
                "LUT_DATA_FILES": self._config.lut_data_filelist
            }
...
#            "AVERAGE_DEM_SLOPE": self._average_dem_slope,
                ...
                if checklist.name == qi:
                    ...
                    for check in checklist.check:
                        for value in check.extraValues.value:
                            ...
                            if value.attrib['name'] == 'CAST_SHADOW_PERCENTAGE' and self._config.productVersion < np.float32(15.0):
                                    value_text = str(switch_case.get(value.attrib['name'], 'NA'))
                                    value.attrib['name'] = 'DARK_FEATURES_PERCENTAGE'
                            else:
                                    value_text = str(switch_case.get(value.attrib['name'],'NA'))
    ...
    def update_qi_metadata(self, switch_case, metadata_id):
        ...
        try:
            ...
            if self._config.productVersion < np.float32(15.0):
                if metadata_id == 'UP2A' or 'T2A':
                    icqi.DARK_FEATURES_PERCENTAGE = switch_case.get('CAST_SHADOW_PERCENTAGE', 'NA')
            else:
                icqi.CAST_SHADOW_PERCENTAGE = switch_case.get('CAST_SHADOW_PERCENTAGE', 'NA')
