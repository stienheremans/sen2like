...
from L2A_Config import L2A_Config, get_script_dir
...
def readNamingConvention(input_dir, mode, logger = None):
        ...
        if mode == 'generate_datastrip':
            ...
            S2A_L1C_mask_safe_standard = 'S2?_OPER_MSI_L1?_DS*'
            S2A_L1C_mask_safe_compact  = 'DS_MPS*'
        elif mode == 'process_tile':
            userProduct = os.path.basename(input_dir)
            S2A_L1C_mask_safe_standard = 'S2?_OPER_MSI_L1?_TL*'
            S2A_L1C_mask_safe_compact  = 'L1?_T*'
        else:
            userProduct = input_dir
            S2A_L1C_mask_safe_standard = 'S2?_OPER_PRD_MSIL1?*.SAFE'
            S2A_L1C_mask_safe_compact  = 'S2?_MSIL1?*.SAFE'
        ...
        if (fnmatch.fnmatch(userProduct, S2A_L1C_mask_safe_standard) == True):
            ...
            nc = 'UNKNOWN'
            if logger:
                logger.error('L1C(H) user product directory must match the following mask: %s' % S2A_L1C_mask_safe_compact)
                logger.error('but is: %s' % userProduct)

        return nc


def parseCommandLine(args, pg = 'NONE'):
    ...
    try:
        ...
        if os.path.exists(input_dir):
            if 'LC08_L1' in input_dir or 'LC09_L1' in input_dir:
                # we have a Landsat 8/9 tile:
                from L2A_Config_Landsat import L2A_Config
                config = L2A_Config(logger, input_dir)
                config.operationMode = 'LANDSAT'
                dummydir, tile_landsat = os.path.split(input_dir)
                config.l1c_tile_id = tile_landsat
                config.init_home_directory()
            else:
                # we are on Sentinel 2
            ...
            # create the new path for the user directory.
                ...
                if config.namingConvention == 'UNKNOWN':
                    if pg != 'NONE':
                        config.namingConvention = 'SAFE_COMPACT'
                        pass
                    else:
                        return config
        else:
    ...
    if (args.mode != 'generate_datastrip' and \
        ...
        config.operationMode != 'TOOLBOX' and \
        'LANDSAT' not in config.operationMode:
    ...
    # end generate_datastrip
        ...
        if os.path.exists(tile):
            ...
            if pg != 'NONE':
                config.final_output_dir = args.final_output_dir
                config.datastrip_dir_l1c = args.datastrip_l1c
                config.metadata_l1c = args.metadata_l1c
            config.datastrip_dir = args.datastrip
            config.granule_dir = args.tile
        else:
            logger.error('Input tile does not exist.')
            return config
    # end process_tile

    if config.operationMode == 'GENERATE_DATASTRIP' or \
       config.operationMode == 'PROCESS_TILE':
    ...
    # end ! TOOLBOX
        ...
        if args.work_dir and 'LANDSAT' in config.operationMode:  # TODO: check if this test S2 / Landsat test can be simplified
            work_dir = os.path.normpath(args.work_dir)
            if not os.path.exists(work_dir):
                os.mkdir(work_dir)
            config.work_dir = work_dir
        else:
            if 'LANDSAT' in config.operationMode:
                config.work_dir = config.output_dir  # TODO: check as the working directory cannot be in the input product directory
            else:
                config.work_dir = directory


        if config.operationMode == 'TOOLBOX':
            # Landsat does not have these features:
            ...
            # step 1 test the standard naming format:
            S2A_L1C_mask = 'S2?_OPER_PRD_MSIL1?*.SAFE'
            if (fnmatch.fnmatch(test, S2A_L1C_mask) == True):
                nc = 'SAFE_STANDARD'
            else:
                S2A_L1C_mask = 'S2?_MSIL1?*.SAFE'
                if (fnmatch.fnmatch(test, S2A_L1C_mask) == True):
                    nc = 'SAFE_COMPACT'
                else:
                    logger.error('L1C(H) user product directory must match the following mask: %s, but is %s' % [S2A_L1C_mask, test])
                    return FAILURE
        # end TOOLBOX
    ...
    if args.resolution == None:
        if 'LANDSAT' not in config.operationMode:
            config.resolution = 0
    else:
    ...
    config.scClassic = args.sc_classic
    config.Hyper  = args.Hyper_MS
    config.scCog   = args.sc_cog
...
def postprocess(config):
    ...
    pg = str(config.get_product_generator_directory())
    if pg != 'NONE':
        import subprocess, shlex, shutil
        granule_l2a_tile = next(os.walk(config.output_dir))[1][0] if next(os.walk(config.output_dir))[1][0][:1] == 'S' else next(os.walk(config.output_dir))[1][1]

        granule_l2a = os.path.join(config.output_dir, granule_l2a_tile)
        granule_l1c = config.get_granule_dir()
        datastrip_l2a = config.get_datastrip_dir()
        datastrip_l1c = config.get_datastrip_dir_l1c()
        metadata_l1c = config.get_metadata_l1c()
        product_l1c = config.input_dir[:-8]
        temp_dir = os.path.join(pg, 'temp_dir')
        out_dir = os.path.join(pg, 'output')
        pg_ds_dir = os.path.join(out_dir, 'DATASTRIP')
        pg_tile_dir = os.path.join(out_dir, 'GRANULE')
        ...
        os.mkdir(pg_ds_dir)
        os.mkdir(pg_tile_dir)

        shutil.move(datastrip_l2a, pg_ds_dir)
        shutil.move(granule_l2a, pg_tile_dir)

        try:
            generateFakeInventory = os.path.join(pg, 'generateFakeInventory.sh')
            if os.path.exists(generateFakeInventory):
                args = ' ' + metadata_l1c + ' '+ os.path.join(pg_ds_dir, next(os.walk(pg_ds_dir))[1][0])
                commandline = generateFakeInventory + args
                p = subprocess.call(shlex.split(commandline))
                if p > 0:
                    config.logger.warning('problem in executing fake Inventory generator')
                    shutil.rmtree(os.path.join(os.getcwd(), 'temp'), ignore_errors=True)
                    shutil.rmtree(temp_dir)
                    shutil.rmtree(out_dir)
                    return False

                args = ' ' + metadata_l1c + ' '+ os.path.join(pg_tile_dir, next(os.walk(pg_tile_dir))[1][0])
                commandline = generateFakeInventory + args
                p = subprocess.call(shlex.split(commandline))
                if p > 0:
                    config.logger.warning('problem in executing fake Inventory generator')
            ...
            productGenerator = os.path.join(pg, 'start_generator.sh')
            if os.path.exists(productGenerator):
                 #-Input/ -input / -I/ -i [User product input dir]: path to the user product to process
                 #-Output/ -output/ -O/ -o [Output directory]: destination directory for complete single tile products
                 #-TemporaryDir [Temporary directory] : temporary directory for products generation
                 #-LogFile [Log file full path]  : path to the log file (Default is ./log/productgenerator.log)
                 #-TileConsolidation [true/false]  :  turn on/off tile consolidation (Default is false)
                 #-MaxThread [max thread number]  : number of parallel pdi to process (Default is 10)
                args = ' -i ' + out_dir
                args += ' -o ' + out_dir
                args += ' -TemporaryDir ' + temp_dir
                #args += ' -LogFile ' + os.path.join(config.logDir,'pg.log')
                #args += ' -TileConsolidation true'
                #args += ' -MaxThread ' + str(config.nrThreads)
                commandline = productGenerator + args
                p = subprocess.call(shlex.split(commandline))

                if p > 0:
                    config.logger.warning('problem in executing product generator')
                    shutil.rmtree(os.path.join(config.get_final_output_dir, 'temp'))
                    shutil.rmtree(temp_dir)
                    shutil.rmtree(out_dir)
                    return False

                shutil.rmtree(temp_dir)
                shutil.rmtree(os.path.join(out_dir, 'DATASTRIP'))
                shutil.rmtree(os.path.join(out_dir, 'GRANULE'))
                shutil.move(os.path.join(out_dir, next(os.walk(out_dir))[1][0]), config.final_output_dir)
                shutil.rmtree(out_dir)
                shutil.rmtree(os.path.join(os.getcwd(), 'temp'), ignore_errors=True)
                return True

        except:
            config.logger.error('Error with product generator check log file for mode details')
    ...
    try:
        ...
        f.flush()
        f.close()
        fnLogOut = os.path.join(config.L2A_UP_DIR, 'rep_info', fnLogBase)
        copyfile(fnLogIn, fnLogOut)
        fnGippIn = config.configFn
        fnGippOut = os.path.join(config.L2A_UP_DIR, 'rep_info', os.path.basename(fnGippIn))
        copyfile(fnGippIn, fnGippOut)
        fnGippIn = config.configSC
        fnGippOut = os.path.join(config.L2A_UP_DIR, 'rep_info', os.path.basename(fnGippIn))
        copyfile(fnGippIn, fnGippOut)
        fnGippIn = config.configAC
        fnGippOut = os.path.join(config.L2A_UP_DIR, 'rep_info', os.path.basename(fnGippIn))
        copyfile(fnGippIn, fnGippOut)
        return True
    except:
...
def main(args=None):
    ...
    ', supporting Level-1C product version ' + str(config.productVersion)
    processorId_landsat = config.processorName +'. Version: '+ config.processorVersion + ', created: '+ config.processorDate + \
    ', supporting Level-1TP Collection_1-2 Landsat_8-9'
    processorId_Hyper = config.processorName +'. Version: '+ config.processorVersion + ', created: '+ config.processorDate + \
    ', supporting Hyper MS Level-1C'
    processorId_help = config.processorName +'. Version: '+ config.processorVersion + ', created: '+ config.processorDate + \
    ', supporting Level-1C product version ' + str(config.productVersion) + ', supporting Level-1TP Collection_1-2 Landsat_8-9' + ', supporting Hyper MS Level-1C'
    ...
    parser = argparse.ArgumentParser(description=processorId_help  + '.', add_help=True)
    ...
    parser.add_argument('--resolution', type=int, choices=[10, 20, 30, 60], help='Target resolution, can be 10, 20 or 60m for S2, 30m for Hyper. If omitted, only 20 and 10m resolutions will be processed')
    parser.add_argument('--datastrip', help='Datastrip folder')
    parser.add_argument('--tile', help='Tile folder')
    parser.add_argument('--output_dir', help='Output directory')
    parser.add_argument('--work_dir', help='Work directory')
    parser.add_argument('--img_database_dir', help='Database directory for L1C(H) input images')
    ...
    parser.add_argument('--sc_only', action='store_true', help='Performs only the scene classification at 60 or 20m resolution, 30m for Hyper')
    parser.add_argument('--sc_classic', action='store_true', help='Performs scene classification in Sen2Cor 2.9 mode')
    parser.add_argument('--sc_cog', action='store_true', help='Export SCL image in COG format instead of JPEG_2000')
    ...
    parser.add_argument('--Hyper_MS', action='store_true', help='To Process a Hyper_MS product')
    # parser.add_argument('--testmode', action='store_true', help='Run with a reduced tile size, configurable in L2A_CAL_AC_GIPP')
    ...
    # detect Product Generator
    xp = L2A_XmlParser(config, 'GIPP')
    node = xp.getNode('Common_Section')
    pg = node.Product_Generator

    datastrip_generated = os.path.exists(os.path.join(config.output_dir, 'temp', 'DATASTRIP'))

    if pg != 'NONE' and not datastrip_generated:
        args.mode = 'generate_datastrip'
        if args.datastrip == None:
            datastrip_dir = os.path.join(args.input_dir, 'DATASTRIP')
            args.datastrip = os.path.join(datastrip_dir, os.listdir(datastrip_dir)[0])
        if args.tile == None:
            granule_dir = os.path.join(args.input_dir, 'GRANULE')
            args.tile = os.path.join(granule_dir, os.listdir(granule_dir)[0])
        if args.processing_centre == None:
            args.processing_centre = 'ABCD'
        if args.archiving_centre == None:
            args.archiving_centre = 'EFGH'
        if args.GIP_L2A == None:
            args.GIP_L2A = os.path.join(config.home, 'cfg', 'L2A_GIPP.xml')
        if args.GIP_L2A_PB == None:
            args.GIP_L2A_PB = os.path.join(config.home, 'cfg', 'L2A_PB_GIPP.xml')
        temporary_dir = os.path.join(config.output_dir, 'temp')
        if not os.path.exists(temporary_dir):
            os.mkdir(temporary_dir)
        args.work_dir = os.path.join(temporary_dir, 'DATASTRIP')
        args.output_dir = args.work_dir
        if not os.path.exists(config.work_dir):
            os.mkdir(config.work_dir)
        config = parseCommandLine(args, pg)
        if args.debug:
            config.logLevel = 'DEBUG'
        if config.operationMode == 'INVALID':
            return FAILURE

    elif pg != 'NONE' and datastrip_generated:
        args.mode = 'process_tile'

        datastrip_dir_l1c = os.path.join(config.output_dir, args.input_dir, 'DATASTRIP')
        converted_datastrip_l1c = next(os.walk(datastrip_dir_l1c))[1][0]
        args.datastrip_l1c = os.path.join(datastrip_dir_l1c, converted_datastrip_l1c)
        args.metadata_l1c = os.path.join(config.output_dir, args.input_dir, 'MTD_MSIL1C.xml')
        datastrip_dir = os.path.join(config.output_dir, 'temp', 'DATASTRIP')
        converted_datastrip = next(os.walk(datastrip_dir))[1][0]
        args.datastrip = os.path.join(datastrip_dir, converted_datastrip)
        if args.tile == None:
            granule_dir = os.path.join(config.output_dir, args.input_dir, 'GRANULE')
            args.tile = os.path.join(granule_dir, os.listdir(granule_dir)[0])
        if args.processing_centre == None:
            args.processing_centre = 'ABCD'
        if args.archiving_centre == None:
        ...
        if args.GIP_L2A_PB == None:
            args.GIP_L2A_PB = os.path.join(config.home, 'cfg', 'L2A_PB_GIPP.xml')
        args.work_dir = os.path.join(config.output_dir, 'temp', 'GRANULE')
        args.final_output_dir = config.output_dir
        args.output_dir = args.work_dir
        if not os.path.exists(config.work_dir):
            os.mkdir(config.work_dir)
        config = parseCommandLine(args, pg)
    ...
    if 'LANDSAT' in config.operationMode:
        config.setTimeEstimation()
        dt = datetime.datetime.now(datetime.UTC).strftime('%Y%m%d')
        if not config.readPreferences():
            return FAILURE

        if not config.create_l2a_tile(dt):
            return FAILURE
        config.init_logger()
        config.logger.stream('%s started ...' % processorId_landsat)
        from L2A_Process_Landsat import L2A_ProcessLandsat
        result = SUCCESS
        tile = L2A_ProcessLandsat(config)
        if tile.process():
            result = SUCCESS
        else:
            result = FAILURE
        ...
        L2A_DS_ID = os.path.join(config.work_dir, config.datastrip.replace('L1?', 'L2A'))
    ...
    if config.operationMode != 'LANDSAT':
        ...
        if  config.Hyper:
            config.logger.stream('%s started ...' % processorId_Hyper)
        else:
            config.logger.stream('%s started ...' % processorId)

    if config.operationMode != 'LANDSAT':
        if float32(config.productVersion) < 14.6:
            config.logger.stream('Old product version %2.1f detected, - will be updated to 14.6' % config.productVersion)
            config.logger.stream('Processing baseline will also be updated')
        else:
            if config.Hyper:
                config.logger.stream('Product version is set to 14.91 for S2P Prisma Products.')
                config.logger.stream('Product version: %2.2f' % (config.productVersion))
            else:
    ...
    if config.operationMode == 'GENERATE_DATASTRIP':
        ...
        if 'LANDSAT' not in config.spacecraftName:
        ...
        if config.operationMode == 'PROCESS_TILE':
            ...
            try:
                ...
                if tile.process() == True:
                    ...
                    result = FAILURE
                if pg != 'NONE':
                    config.set_product_generator_directory(pg)
    ...
    if pg != 'NONE' and result == SUCCESS and not datastrip_generated:
        main()
