...
class L2A_Manifest():
    def __init__(self, config):
        ...
        if config.namingConvention == 'SAFE_STANDARD': 
            self.DICO_IDS = { 
            # PRODUCT ROOT -->
                "S2_Level-2A_Product_Metadatar": "S2[AB]_OPER_MTD_SAFL2A_PDMC",
            ...
            # GRANULE-->
                "S2_Level-2A_Tile([1-9]?[0-9]{1,6})_Metadata": r"S2[AB]_OPER_MTD_L2A_TL__........T......_......._......\.xml",
                #"S2_Level-2A_Tile([1-9]?[0-9]{1,6})_Metadata": "S2[AB]_OPER_MTD_L2A_TL__20151021T153950_A001515_T53LNA.xml",
            # GRANULE/IMG_DATA -->
                "IMG_DATA_10m_Band[1-4]_Tile([1-9]?[0-9]{1,6})_Data": r"S2[AB]_OPER_MSI_L2A_TL_.*_B\d\w_10m.jp2",
                "IMG_DATA_20m_Band[1-9]_Tile([1-9]?[0-9]{1,6})_Data": r"S2[AB]_OPER_MSI_L2A_TL_.*_B\d\w_20m.jp2",
                "IMG_DATA_60m_Band([1-9]|1[0-2])_Tile([1-9]?[0-9]{1,6})_Data": r"S2[AB]_OPER_MSI_L2A_TL_.*_B\d\w_60m.jp2",
            ...
            # GRANULE/IMG_DATA -->
                "IMG_DATA_10m_Band[1-4]_Tile([1-9]?[0-9]{1,6})_Data": r"L2A_T.*_B\d\w_10m.jp2",
                "IMG_DATA_20m_Band[1-9]_Tile([1-9]?[0-9]{1,6})_Data": r"L2A_T.*_B\d\w_20m.jp2",
                "IMG_DATA_60m_Band([1-9]|1[0-2])_Tile([1-9]?[0-9]{1,6})_Data": r"L2A_T.*_B\d\w_60m.jp2",
            ...
            # end fix for SIIMPC-734
                "S2_Level-1C_Preview_Tile([1-9]?[0-9]{1,6})_Data": os.path.join('QI_DATA', 'T.*_PVI.jp2').encode('unicode-escape').decode(),
                "S2_Level-2A_Preview_Tile([1-9]?[0-9]{1,6})_Data": os.path.join('QI_DATA', 'L2A_T.*_PVI.jp2').encode('unicode-escape').decode(),
    ...
    def listToIds(self, dataObjectSection, s2alist):
        ...
        """
        ids={}
        for key, pattern in self.DICO_IDS.items():
    ...
    def getChecksum(self, filename):
        ...
        if os.path.exists(filename):
            if filename.split(".")[-1] == 'jp2':
                ...
                return md5(open(filename, 'rb').read()).hexdigest()
                # return md5(open(filename, 'r').read().encode('UTF-8')).hexdigest()
        else:
            return None
    ...
    def addDataObject(self, parent, href, id, l2productpath):
        ...
        l2productpath_ = str(os.path.join(l2productpath, href))
    ...
    def generate(self, l2productpath, safeL1cFn):
        ...
        for key in list(self.NSMAP.keys()):
            ...
            if filepath in ids:
                ...
                if tilename is not None:
                    ...
                    if tilename not in tilemap:
                        tileid = len(list(tilemap.keys()))+1
                ...
                ## DATASTRIPS
                    ...
                    if stripname not in stripmap:
                        ...
                        stripid = len(list(stripmap.keys()))+1
        ...
        for childL1C in  metadataSectionL1C.getchildren():
            if "ID" in list(childL1C.attrib.keys()) and childL1C.attrib["ID"] in L1C_KEEP_LIST:
        ...
        with open(fn, "w") as o:
            o.write(ET.tostring(root, encoding='UTF-8', pretty_print=True, xml_declaration=True).decode())
